...
class GraphSage(torch.nn.Module):
    def __init__(self, args):
        ...
        if args.aug == False:
            ...
            self.conv1 = SAGEConvNew1(
                dataset['num_node_features'], dim_size)
            self.conv2 = SAGEConvNew1(dim_size, dataset['num_classes'])
...
class SAGEConvNew1(MessagePassingNew):
    def __init__(self, in_channels: Union[int, Tuple[int, int]],
                 out_channels: int, normalize: bool = False,
                 root_weight: bool = True,
                 bias: bool = True, negative_slope: float = 0.2, **kwargs):  # yapf: disable
        kwargs.setdefault('aggr', 'mean')
        super(SAGEConvNew1, self).__init__(**kwargs)
        ...
        if isinstance(in_channels, int):
            self.lin_1 = GCNConv(in_channels, out_channels, bias=False)
            self.lin_2 = self.lin_1
        else:
            print(in_channels[0], out_channels)
            self.lin_1 = GCNConv(in_channels[0], out_channels, False)
            self.lin_2 = GCNConv(in_channels[1], out_channels, False)

        self.att_l = Parameter(torch.Tensor(1, out_channels))
        self.att_r = Parameter(torch.Tensor(1, out_channels))

        if out_channels % 2 == 0:
            self.lin_p = GCNConv(in_channels[0], int(
                out_channels / 2), bias=bias)
            self.lin_n = GCNConv(in_channels[1], int(
                out_channels / 2), bias=bias)
        else:
            self.lin_p = GCNConv(in_channels[0], int(out_channels), bias=bias)
            self.lin_n = GCNConv(in_channels[1], int(out_channels), bias=bias)
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        if isinstance(x, Tensor):
            assert x.dim() == 2, 'Static graphs not supported.'
            x_l = x_r = self.lin_1(x, edge_index).view(-1, C)
            sigma_l = (x_l * self.att_l).sum(dim=-1)
            sigma_r = (x_r * self.att_r).sum(dim=-1)
        ...
        out, sigma = self.propagate(edge_index, x=x, sigma=(
            sigma_l, sigma_r), size=[None, None])
        out = self.lin_p(out, edge_index)

        out_, _ = self.propagate(edge_index, x=x, sigma=(
            sigma_l, sigma_r), size=[None, None], previous=sigma)

        if self.out_channels % 2 == 0:
            out = torch.cat((out, self.lin_n(out_, edge_index)), 1)
        else:
            out += self.lin_n(out_, edge_index)
