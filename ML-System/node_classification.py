import torch
from torch.nn import DataParallel, CrossEntropyLoss, Sequential, Module
...
def test(args, model, mask_type="test", return_sigma_acc=False):
    ...
    # print('{} Accuracy: {:.4f}'.format(mask_type, acc))
...
def main():
    ...
    # experiment_ans["datasetCategory"].append(category_name)
    ...
    if model_name != "GraphSage":
        ...
        args.dataset["graph"][0].edge_index = edge_index
    else:
    ...
    df.to_csv(
        f'./saved/log/{args.dataset_name}/lr{args.lr}_weightdecay{args.weight_decay}_lamda{args.lamda}_dataset{args.dataset_name}_model{args.model_type}_split{args.split}_aug{args.aug}_seed{args.seed}.csv')
