...
class GCNConvNewUpperBound(MessagePassing):  # temp for upper bound

    _cached_edge_index: Optional[Tuple[Tensor, Tensor]]
    _cached_adj_t: Optional[SparseTensor]

    def __init__(self, in_channels: int, out_channels: int, similarity,
                 improved: bool = False, cached: bool = False,
                 add_self_loops: bool = True, normalize: bool = True,
                 bias: bool = True,
                 **kwargs):
        ...
        kwargs.setdefault('aggr', 'add')
        super(GCNConvNewUpperBound, self).__init__(**kwargs)
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.improved = improved
        self.cached = cached
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_weight: OptTensor = None) -> Tensor:
        ...
        out = self.propagate(edge_index, edge_weight=edge_weight, x=(x_p, x_n),
                             size=None)
    ...
    def message(self, x_j: Tensor, x_i: OptTensor, edge_weight: OptTensor) -> Tensor:
        sigma = torch.tensor(self.similarity)
        return x_j if edge_weight is None else edge_weight.view(-1, 1) * x_j * sigma.clone().view(-1, 1) + edge_weight.view(-1, 1) * x_i * (1 - sigma.clone().view(-1, 1))
